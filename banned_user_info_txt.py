...
def write_banned_user_info(role_list, user_id): 
    banned_user_info_file = open("banned_user_info.txt", "a") #otwarcie pliku
    try:
        print("Writting user info to file ...")
        banned_user_info_file.write(str(user_id)+ "\n")
        ...
        for role in role_list : #zapisanie r�l u�ytkownika w txt
            banned_user_info_file.write(str(role.name)+ "\n")
            print(role.name)
    finally:
        print("File closed, user info saved")
        banned_user_info_file.close()

async def read_banned_user_info(client1): #odczyt pliku z info zbanowanych u�ytkownik�w
    print("Checking banned_user_info file ...")
    filesize = os.path.getsize("banned_user_info.txt") #rozmiar pliku txt
    if filesize == 0: #je�eli plik jest pusty funkcja nic nie robi
        print("The file is empty")
    else:
        print("Restoring roles to users ...")
        banned_user_info_file = open("banned_user_info.txt", "r") #otwarcie pliku
        try:
            guild = client1.get_guild(470919364951146507) #zapis obiektu gildii
            for line in banned_user_info_file.readlines():
            ...
            banned_user_info_file.close()
            open('banned_user_info.txt', 'w').close() #otwarcie i zamkni�cie pliku w celu usuni�cia jego zawarto�ci
